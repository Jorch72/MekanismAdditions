package chaos234.mekanismadditions.common.recipe;

import chaos234.mekanismadditions.MekanismAdditions;
import chaos234.mekanismadditions.common.util.RecipeUtils;
 ...
 */
public class ShapedMekanismAdditionsRecipe implements IRecipe{
    ...
    public ShapedMekanismAdditionsRecipe(ItemStack result, Object... recipe)
    {
        output = result.copy();
    ...
    public ShapedMekanismAdditionsRecipe setMirrored(boolean mirror)
    {
        mirrored = mirror;
        return this;
    }
    ...
    public static ShapedMekanismAdditionsRecipe create(NBTTagCompound nbtTags)
    {
        if(!nbtTags.hasKey("result") || !nbtTags.hasKey("input"))
        {
            MekanismAdditions.maLogger.error("[Mekanism Additions] Shaped recipe parse error: missing input or result compound tag.");
            return null;
        }
        ...
        {
            MekanismAdditions.maLogger.error("[Mekanism Additions] Shaped recipe parse error: invalid result stack or input data list.");
            return null;
        }
        ...
        for(int i = 0; i < list.tagCount(); i++)
        {
            ...
            else if(compound.hasKey("character"))
            {
                ...
                if(s.length() > 1)
                {
                    MekanismAdditions.maLogger.error("[Mekanism] Shaped recipe parse error: invalid pattern character data.");
                    return null;
                }
            ...
            else if(compound.hasKey("itemstack"))
            {
                ...
                MekanismAdditions.maLogger.error("[Mekanism] Shaped recipe parse error: invalid input tag data key.");
                return null;
            }
        }

        return new ShapedMekanismAdditionsRecipe(result, ret);
    }
}
